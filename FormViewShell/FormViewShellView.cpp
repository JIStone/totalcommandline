...
void CFormViewShellView::OnInitialUpdate()
{
	...
	if(!m_IsToolTipInit)
	{
		...
		m_toolTip.AddTool(GetDlgItem(IDC_EDIT7), _T("파일처리후 최종확장자 ex) .nlz"));
...
void CFormViewShellView::OnBnClickedExecappl()
{
	...
	{ 
		AfxMessageBox(_T("변환할 파일을 선택하세요."));
			...
			for(int lbIndex = 0; lbIndex < m_ListBox_Out.GetCount(); lbIndex++)
			{
				...
				{	
					...
					else if(iExtType == FO_DELETE)
					{
						...
						//CStringW myTestStr(L"(C93) [はよしねま (越山弱衰)] 都橋先生は今日も釦を飛ばす。 (オリジナル).torrent");
						//CString myTestStr_("(C93) [はよしねま (越山弱衰)] 都橋先生は今日も釦を飛ばす。 (オリジナル).torrent");
						//
...
*/
				...
				if(bAtiveFileProc)
				{
					...
					{
						//파일처리 리스트 출력하기
						CString fileOp[3]= {_T(" 이동"), _T(" 복사"), _T(" 삭제")};
						if(iExtType == FO_DELETE)
						{
							m_ExcuteFilePath +=	movingStr + _T(" 를") + fileOp[iExtType - 1] + _T("\r\n");
						}
						else
						{
							m_ExcuteFilePath +=	movingStr + _T(" 를") + _T("  \r\n")  + movDest + _T("로") + fileOp[iExtType - 1] + _T("\r\n");
						}
					...
					isDestFileExist = PathFileExists(movDest);// 처리할 파일(결과파일)이 경로에 있는지 검사
					if(!isDestFileExist && m_Check_EnableErrPop.GetCheck())
					{
						int popRet = MessageBox(movDest + _T(" \n파일이 없습니다. 계속 하시겠습니까?"), _T("파일처리 에러"), MB_OKCANCEL);
					...
					{
						int popRet = MessageBox(movDest + _T(" \n작업을 마치지 못했습니다. 계속 하시겠습니까?"), _T("파일처리 에러"), MB_OKCANCEL);
			...
			if(bUseShellExCute)
			{
				switch((int)ret)
				{
					case SE_ERR_NOASSOC:
					{
						MessageBox(_T("지정된 파일 확장자로 실행할 수 있는 프로그램이 없습니다."), _T("파일 확장자 오류"), MB_OKCANCEL);
					}
					break;
					case SE_ERR_FNF:
					{
						MessageBox(_T("지정된 파일을 찾을수 없습니다."), _T("파일 오류"), MB_OKCANCEL);
					}
					break;
				}
			}
	...
	// 결과 표시
	CString resultCnt;
	resultCnt.Format(_T("%d / %d 성공"), iSuccessCnt, progressCnt);
...
void CFormViewShellView::OnBnSeqExcuteButton()
{
	...
	if(m_MTCL_ExitCheckBox.GetCheck()){
		MessageBox(_T("==== 완료! ===="), _T("TotalCommandLine"));
		exit(0);
	}
}
/*
	...
	if(!isSuccess)
	{
		...
		switch(err)
		{
			case ERROR_FILE_NOT_FOUND:
			{
				...
				MessageBox(_T("지정된 파일을 찾을수 없다."));
			}
			break;
			case ERROR_DDE_FAIL:
			{

				MessageBox(_T("동적 데이터 교환(DDE) 트랜잭션(transaction)에 실패했습니다."));
			}
			break;
			case ERROR_NO_ASSOCIATION:
			{

				MessageBox(_T("주어진 파일의 확장명과 연결되는 응용 프로그램이 없다."));
			}
			break;
			case ERROR_ACCESS_DENIED:
			{

				MessageBox(_T("지정된 파일에 대한 액세스가 거부 되었다."));
			}
			break;
			case ERROR_DLL_NOT_FOUND:
			{

				MessageBox(_T("응용 프로그램을 실행하는 데 필요한 하나의 라이브러리(library)  파일을 찾을 수없습니다."));
			}
			break;
			case ERROR_CANCELLED:
			{

				MessageBox(_T("함수가 사용자를 위한 추가정보를 알려주었지만 사용자가 요청을 취소 했다."));
			}
			break;
			case ERROR_NOT_ENOUGH_MEMORY:
			{

				MessageBox(_T("그곳에 지정된 작업을 수행하기 위한 충분한 메모리가 없다."));
			}
			break;
			case ERROR_SHARING_VIOLATION:
			{

				MessageBox(_T("공유 위반이 발생했다."));
			}
			break;
		}
	}
